...
public class BaiduShiWu extends Activity {
	...
	private ImageView dingwei, thinglocation;// 点击获取当前定位坐标，我的位置按钮，点击获取我现在所在的位置
	// 覆盖物相关
	private BitmapDescriptor mMarker;
	// 编辑框布局,初始化时隐藏该布局
	private RelativeLayout relativeLayout;

	String time, created, place, label, elatitude, langitude, textdetail,
			title;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		textdetail = bundle.getString("textdetail");
		elatitude = bundle.getString("latitude");
		langitude = bundle.getString("langitude");
		title = bundle.getString("title");

		// 从数据库获取的经纬度
		latitude1 = Double.valueOf(elatitude.toString());// 把内容是数字的字符串强转为double类型
		longitude1 = Double.valueOf(langitude.toString());// 把内容是数字的字符串强转为double类型

//		System.out.println("点击地图图标获取过来的数据：" + time + "创建时间：" + time + place
//				+ label + textdetail + langitude + elatitude);
		...
		// 手動定位到當前位置
		thinglocation = (ImageView) findViewById(R.id.thingslocation);
		thinglocation.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
	...
	private void addOverlays() {
		...
		LatLng latLng1 = new LatLng(latitude1, longitude1); // 是用户选定的经纬度
		mMarker = BitmapDescriptorFactory.fromResource(R.drawable.mark);
		// 图标
	...
	private void init() {
		...
		mBaiduMap.setOnMarkerClickListener(new OnMarkerClickListener() {
			...
			public boolean onMarkerClick(Marker marker) {
				...
				TextView title1 = (TextView) relativeLayout
						.findViewById(R.id.title);
				TextView time1 = (TextView) relativeLayout
				...
				TextView label1 = (TextView) relativeLayout
						.findViewById(R.id.label);
				// Button
				// button=(Button)relativeLayout.findViewById(R.id.check_details);

				time1.setText(created);
				title1.setText(title);
				lost_time.setText(time);
				lost_place.setText(place);
				thing_details.setText(textdetail);
				// label1.setText(label);

				Lable.lable(label, label1);
