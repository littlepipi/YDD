...
import app.example.activity.AccountSettingActivity;
import app.example.activity.DiscoverRecord;
import app.example.activity.FeedbackActivity;
import app.example.activity.FoundRecord;
import app.example.activity.LostRecord;
...
public class FragmentMine extends Fragment implements Callback {
	...
	private TextView Nickname, Location;
	private String personname, personportrait, location;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		new Thread() {
			public void run() {
				...
				UIHandler.sendEmptyMessageDelayed(1, 100, FragmentMine.this);
			}
		}.start();
		/**********************************************/
		head_portrait.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intentportrait = new Intent(getActivity(),
		...
		relative = (RelativeLayout) view.findViewById(R.id.screen_hight);
		relative.getLayoutParams().height = mScreenWidth;

		GridView gridview = (GridView) view
				.findViewById(R.id.fragment_mine_gridview);
		...
		gridview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				switch (arg2) {
				case 0:// 失物记录
					Intent intent0 = new Intent(getActivity(), LostRecord.class);
				...
				case 4:
					...
					boolean net2 = CheckNet.detect(getActivity());
	...
	private void init() {
		...
		imageLoader = new ImageLoader(getActivity());// 头像加载器

		SharedPreferences MyHttpPerson = MyApplication.getAppContext()
				.getSharedPreferences("MyHttpPerson", Activity.MODE_PRIVATE);

		personname = MyHttpPerson.getString("personname", "").toString();

		personportrait = MyHttpPerson.getString("personportrait", "");

		location = MyHttpPerson.getString("personlocation", "").toString();

		Nickname.setText(personname);
		Location.setText(location);
		imageLoader.DisplayImage(personportrait, head_portrait);
	...
	public void onComplete(Platform plat, int action,
		...
		msg.arg2 = action;
		msg.obj = plat;
		UIHandler.sendMessage(msg, FragmentMine.this);
	}
...
//		String Urlt = "http://123.56.150.30:8000/static/release/update/YiDiudiu.apk";
		...
		oks.setSiteUrl(Urlt);
		oks.setImageUrl(FragmentMine.this.TEST_IMAGE_URL);
		oks.setFilePath(FragmentMine.this.TEST_IMAGE);
