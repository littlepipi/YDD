...
public class HttpUI {
	...
	String id, what, losttime, lostlatitude, lostlangitude, kind, information,
			...
			followingnumber, location, created, title,  latlng, image1, image2, image3;

	private int i;
	
	public HttpUI(int start, int end, String address, String httpkind,String somekind) {
		this.start = start;
		this.end = end;
...
//        System.out.println("网络超时处理------>"+  client.getTimeout());
		String urlr = UrlPath.messagesGetApi;

		String url = urlr + "?"  + "location="
				+ address + "&start=" + start + "&end=" + end+"&what=" + httpkind +"&kind="+somekind;
		...
		client.get(url, new JsonHttpResponseHandler() {
				...
				if (httpkind.equals("lost")) {
					 int a=response.length();
						SharedPreferences mySharedPreferences =
						 ...
						 "HttpUI_lost", Activity.MODE_PRIVATE);
						 // 实例化SharedPreferences.Editor对象（第二步）
						 SharedPreferences.Editor editor = mySharedPreferences.edit();
						 // 用putString的方法保存数据
						 editor.putInt("HttpUI_lost", a);
						 // 提交当前数据
						 editor.commit();
					 ...
					 int a=response.length();
						SharedPreferences mySharedPreferences =
						 MyApplication.getAppContext().getSharedPreferences(
						 "HttpUI_find", Activity.MODE_PRIVATE);
						 // 实例化SharedPreferences.Editor对象（第二步）
						 SharedPreferences.Editor editor = mySharedPreferences.edit();
						 // 用putString的方法保存数据
						 editor.putInt("HttpUI_find", a);
						 // 提交当前数据
						 editor.commit();
				}
				...
				try {
					...
					for (i = 0; i < response.length(); i++) {
						...
						comments = null;
							...
							HttpUIPerson httpPersonMessage = new HttpUIPerson(
									commitperson);
							httpPersonMessage.submitAsyncHttpClientGet(i, id,
									what, losttime, lostlatitude,
									lostlangitude, kind, information,
									commitperson, contactphone, sharenumber,
									commentsnumber, followingnumber, location,
									created, title, comments, image1, image2,
									image3);
			...
			public void onFailure(int statusCode, Header[] headers,
					Throwable throwable, JSONObject errorResponse) {if (httpkind.equals("lost")) {
