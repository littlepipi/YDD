...
import app.example.activity.RegisterPersonalPageActivity;
...
//				System.out.println("点击下一步获取------------");

				// 在这里需要上传验证码
				AsyncHttpClientGet(phone, verificationCode);
		...
		if (resId > 0) {
			...
			tv.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					// // 提交验证
					// String verificationCode = etIdentifyNum.getText()
					// .toString().trim();
					// // 在这里需要上传验证码
					//
					// System.out.println("点击重新获取获取------------");
					// AsyncHttpClientGet(phone, verificationCode);
			...
			public void run() {
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					...
					Intent intent = new Intent(activity,
							RegisterPersonalPageActivity.class);
					activity.startActivity(intent);

					Toast.makeText(activity, "验证成功！", Toast.LENGTH_SHORT)
							.show();
					activity.finish();
	...
	/**********************************************************/
	protected void AsyncHttpClientGet(final String phone_number,
			final String identify) {
		// TODO Auto-generated method stub
		// 发送请求到服务器
		AsyncHttpClient client = new AsyncHttpClient();

		String urlr = UrlPath.identifyOperateApi;

		String url = urlr + phone_number + "&identify=" + identify;

//		System.out.println("上传数据验证码--------->" + phone_number + "----"
//				+ identify);
		// 执行post方法
		client.get(url, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				SharedPreferences mySharedPreferences1 = MyApplication
						.getAppContext().getSharedPreferences("messgae",
								Activity.MODE_PRIVATE);
				// 实例化SharedPreferences.Editor对象（第二步）
				SharedPreferences.Editor editor1 = mySharedPreferences1.edit();
				// 用putString的方法保存数据
				editor1.putString("phone_number", phone_number);
				editor1.putString("verificationCode", messgae);
				// 提交当前数据
				editor1.commit();
...
//				System.out.println("下一步失败失败statusCode---------->" + statusCode);
				Toast.makeText(activity, "无效验证码", Toast.LENGTH_LONG).show();
//				String messgae = new String(responseBody);
